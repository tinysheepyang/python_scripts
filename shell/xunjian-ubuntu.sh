...
ubuntuVersion=$(awk '{print $2}' /etc/issue)
VERSION=`date +%F`
#日志相关
LOGPATH="/tmp/awr"
...
function getSystemStatus(){
    ...
    Release=$(cat /etc/issue 2>/dev/null)
    Kernel=$(uname -r)
    OS=$(uname -o)
    Hostname=$(uname -n)
...
function getServiceStatus(){
    ...
    echo ""
    conf=$(chkconfig --list| grep -E ":on|:启用")
    process=$(service --status-all 2>/dev/null | grep -E "+")
